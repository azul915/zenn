...
- 小規模チーム向けの現実解は、**CIS Controls 等のベースライン**と**事業被害ベースの攻撃シナリオ**を組み合わせる二軸分析
- 6段階（影響度定義 → プロファイル → 二軸分析 → 統合 → 対応）を回すと、対策タスクに優先度（P1–P4）まで付けられる
- 分析で終わらせない。**リスクレジスタに P1 の担当と期限を1件入れた瞬間**から運用フェーズ

## 専任担当がいなくても、プロセスは回せる

セキュリティリスク分析は大企業や行政の話に聞こえがちです。ただ、専任のセキュリティ担当がいない小規模チームでも、手順を固定しておけば対策の優先順位を説明できます。

本記事では、B2B SaaS（クラウドバックエンド＋Webフロント＋現場端末＋外部SaaS連携）向けに、政府系セキュリティ分析ガイドラインを参考に実施したリスク分析の**手順**だけを、固有名詞を抽象化して書きます。特定プロダクトの評価結果そのものではありません。
...
小規模チームなら、**ベースラインを軸に、事業被害ベースで穴を埋める**のが現実的です。政府系ガイドラインでも同じ構成が推奨されています。

この組み合わせが満たすのは次の3点です。

1. 想定される脅威と対策を一通り把握できる（網羅性）
2. 技術・事業・評価の3視点で相互確認できる（レビュー可能性）
3. 限られた人員でも完走できる（現実的な工数）
...
## Step 1: 先に物差しを作る——事業への影響度

後続のすべての工程で使う共通基準を、最初に決めます。

### 事業リスクの種類（例）

ガイドラインでは、次の5類型が定義されています（文言は要約）。
...
- 四半期〜年次で見直す。一度作って終わりにしない

---

## Step 2: 前提を固定する——システムプロファイル
...
「目標の AAL」と「現状の AAL」の両方を書いておくと、ギャップが対策タスクに直結します。

### 粒度のコントロール

- 完璧な設計書を目指さない。レビューに必要な粒度で十分
- 未確定の仕様は「TBD」と明記し、バックログで追跡する
- 作成自体が目的化しない。分析の前提を固定することが目的

---

## Step 3: 横断チェック——ベースラインアプローチ
...
ベースラインは「一般的に必要な対策」の網羅に向いています。一方で、自社固有の業務フロー上のリスクは拾いにくく、「対策済」判定が運用実態とズレやすい（ポリシーはあるが未徹底、など）。この穴を Step 4 で埋めます。

---

## Step 4: 自社固有のリスク——事業被害ベースアプローチ

Step 1 で定義した事業被害を起点に、攻撃シナリオ → 攻撃ステップ → リスク値 → 対策、の順でたどります。

### 分析対象の分割

SaaS では攻撃面がレイヤーごとに分かれます。表もレイヤー単位で分けると整理しやすいです。
...
結果を A(9) / A(6) / B(2) 等の帯域にマッピングし、A(9) 以上を優先検討とする、といった運用ルールを決めておきます。
...
4. Step 3 の「必要」項目と必ず紐付ける。二重管理を防ぐ

---

## Step 5: タスクに落とす——リスク分析の取りまとめ
...
P1 から着手します。スコアが高く、複数シナリオに効く対策（認証強化、Secrets 管理、端末暗号化など）が典型です。P2 は四半期計画に載せ、P3 / P4 はバックログに入れます。「許容」と決めた項目も定期レビューは必要です。
...
SARIF を共通形式にすると、複数ツールの結果を1チャネル（Slack 等）に集約しやすくなります。Step 4 の「アプリケーション脆弱性」シナリオに対する継続的な検知として位置づけられます。
...
## 実際に回してみて——うまくいった点とハマりどころ

二軸分析にしたおかげで、「一般的な不足」と「自社固有リスク」の両方を説明できるようになりました。レイヤー分割（クラウド / エッジ / 外部 SaaS）にしたのも、レビュー担当者が分担しやすかったです。優先度スコアを入れたことで「なぜ P1 か」の会話もしやすくなり、ベースラインと CI の SAST を Control 番号で接続できたのも収穫でした。
...
6ステップ全部を一度に完走する必要はありません。次の順が現実的です。
...
小規模 SaaS でも、ベースライン＋事業被害ベースの二軸分析は回せます。やることはシンプルで、先に物差し（事業リスクとプロファイル）を作り、一般と固有を分けて洗い、スコアで優先順位を決め、リスクレジスタで運用に移す——この4点です。

固有名詞やプロダクト仕様を抽象化しても、プロセスの骨格はそのまま転用できます。完璧な分析表より、P1 の1件に担当と期限が入ったリスクレジスタのほうが、チームにとってははるかに価値があります。Step 1 から小さく始めてみてください。
...
自社用に書き換えれば、社内ドキュメントのテンプレートとしても使えます。
